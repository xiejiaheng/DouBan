            ...
            <div class="mui-card-content">
                <ul><li class="mui-table-view-cell mui-media">
					<router-link :to="{path: '/home/Movieinfo', query: {id: item.subject.id }}">
                        ...
                        <div class="mov">
                            &nbsp;&nbsp;豆瓣评分：{{item.subject.rating.average}}<br>
                        &nbsp;&nbsp;类型：{{item.subject.genres[0]}}<br>
                        &nbsp;&nbsp;导演：{{item.subject.directors[0].name}}<br>
                        &nbsp;&nbsp;主演：{{item.subject.casts[0].name}}...<br>
...
export default {
    ...
    methods:{
        gettoplist(){
            this.$http.get('api/movie/weekly?apikey=0df993c66c0c636e29ecbb5344252a4a').then(function(respon){
